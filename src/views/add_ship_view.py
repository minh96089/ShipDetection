...
class AddShipView(tk.Toplevel):

    def __init__(self, parent, class_options):
        super().__init__(parent)
        self.title('Thêm tàu mới')
        self.geometry('550x650')
        self.resizable(False, False)
        self.grab_set()
        self.colors = {'bg': '#f8f9fa', 'primary': '#3498db', 'success': '#2ecc71', 'text': '#2c3e50'}
    ...
    def setup_ui(self):
        ...
        tk.Label(form_frame, text='Số hiệu tàu (*)', font=('Segoe UI', 11, 'bold'), bg='white', fg=self.colors['text']).pack(anchor='w', pady=(0, 5))
        ...
        tk.Label(form_frame, text='Ảnh đại diện', font=('Segoe UI', 11, 'bold'), bg='white', fg=self.colors['text']).pack(anchor='w', pady=(0, 5))
        ...
        save_btn = tk.Button(btn_frame, text='✅ THÊM TÀU', bg=self.colors['success'], fg='white', font=('Segoe UI', 12, 'bold'), bd=0, padx=30, pady=10, cursor='hand2', command=self.on_save_clicked)
    ...
    def create_header(self):
        ...
        tk.Label(header, text='⛴️ THÊM TÀU MỚI', font=('Segoe UI', 16, 'bold'), bg=self.colors['primary'], fg='white').pack(pady=15)
    ...
    def get_data(self):
        ...
        anh_dai_dien = self.temp_image_path.get()
        mo_ta = self.e_description.get('1.0', tk.END).strip()
        if not mo_ta:
            mo_ta = 'không có mô tả'
        return {'so_hieu': so_hieu, 'class_name': class_name, 'anh_dai_dien': anh_dai_dien, 'mo_ta': mo_ta}
