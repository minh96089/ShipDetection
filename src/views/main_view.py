...
from src.controllers.auth_controller import AuthController, Permission, CurrentUser
...
class MainView:
    ...
    def setup_navbar(self):
        ...
        # Các tab luôn hiển thị cho mọi người dùng
        tk.Button(navbar, text='Hệ thống giám sát', **nav_style, command=lambda: self.show_frame('monitoring')).pack(side=tk.LEFT)
        if AuthController.has_permission(Permission.VIEW_LOGS):
            tk.Button(navbar, text='Nhật ký phát hiện', **nav_style, command=lambda: self.show_frame('database')).pack(side=tk.LEFT)
        # Chỉ admin có quyền MANAGE_SHIPS mới thấy tab Quản lý tàu
        if AuthController.has_permission(Permission.MANAGE_SHIPS):
            tk.Button(navbar, text='Quản lý tàu', **nav_style, command=lambda: self.show_frame('ships')).pack(side=tk.LEFT)
        # Cả user và admin đều xem được Vi phạm
        if AuthController.has_permission(Permission.VIEW_ALERTS):
            tk.Button(navbar, text='Quản lý Vi phạm', **nav_style, command=lambda: self.show_frame('alerts')).pack(side=tk.LEFT)
        # Chỉ admin có quyền VIEW_REPORTS mới thấy tab Báo cáo
        if AuthController.has_permission(Permission.VIEW_REPORTS):
            tk.Button(navbar, text='Báo cáo', **nav_style, command=lambda: self.show_frame('reports')).pack(side=tk.LEFT)
        # Chỉ admin có quyền VIEW_STATISTICS mới thấy tab Thống kê
        if AuthController.has_permission(Permission.VIEW_STATISTICS):
    ...
    def show_frame(self, page_name):
        ...
        page_permission_map = {
            'database': Permission.VIEW_LOGS,
            'ships':    Permission.MANAGE_SHIPS,
            'reports':  Permission.VIEW_REPORTS,
            'statistics': Permission.VIEW_STATISTICS,
        }
        required_permission = page_permission_map.get(page_name)
        if required_permission and not AuthController.has_permission(required_permission):
            from tkinter import messagebox
            messagebox.showwarning('Quyền hạn chế', f'Bạn không có quyền truy cập trang này.\nLiên hệ quản trị viên để được cấp quyền.')
            return
    ...
    def setup_monitoring_page(self):
        ...
        self.canvas_video.bind('<Button-1>', self._on_canvas_left_press)
        self.canvas_video.bind('<B1-Motion>', self._on_canvas_left_drag)
        self.canvas_video.bind('<ButtonRelease-1>', self._on_canvas_left_release)
